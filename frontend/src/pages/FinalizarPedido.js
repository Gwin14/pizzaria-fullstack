...
function FinalizarPedido({ onPedidoFinalizado }) {
  ...
  useEffect(() => {
    ...
    // Buscar todas as pizzas para mostrar nome no resumo
    pizzaAPI.getAll().then(setPizzas);

    if (!clienteId) {
      setMensagem("❌ Erro: Faça login para finalizar seu pedido.");
      setTipoMensagem("error");
    }
  }, []);

  // Função utilitária para pegar o nome da pizza pelo id
  const getPizzaNome = (pizzaId) => {
    const pizza = pizzas.find((p) => p.id === pizzaId);
    return pizza ? pizza.nome : `Pizza #${pizzaId}`;
  };
        ...
        <div className="pizzeria-item-card">
          ...
          <div style={{ display: "grid", gap: "10px" }}>
            {pedido.itens?.map((item) => (
              ...
              >
                <strong>{getPizzaNome(item.pizzaId)}</strong> - Quantidade: {item.quantidade} -
